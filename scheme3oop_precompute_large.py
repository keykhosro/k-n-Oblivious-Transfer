...
class Sender:
    def __init__(self,M,n):
        self.bulletin, self.d, self.x,self.C = self.sender_setup(M,n)

    def sender_setup(self,M,n):
        ...
        C = []
        for i in range(n):
            C.append((M[i] * self.x * pow(primeset[i], d, N)) % (N))
        return bulletin, d, x,C

    def sender_step1(self, R1):
...
for i in range(1000):
    ...
    C, S1 = sender.sender_step1(receiver.R1)
